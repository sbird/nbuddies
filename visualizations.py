...
def movie_3D(tail_length: int = 10, tot_nstep_eta = None, brute_or_tree = None):
    """
    ...
    if not os.path.exists(nbuddies_path+"/movie_dump"+brute_or_tree): # check if dir exists
        os.makedirs(nbuddies_path+"/movie_dump"+brute_or_tree) # if not, create dir path
    ...
    for i in range(last_batch_num + 1):
        ...
        plt.savefig(nbuddies_path + "/movie_dump"+brute_or_tree+f"/trajectories_{i}.png", dpi=300, bbox_inches='tight') # save fig in dir
        plt.close()

    _recompile_movie_3D(tot_nstep_eta, brute_or_tree) # Combine saved frames into video using ffmpeg

def _recompile_movie_3D(tot_nstep_eta, brute_or_tree):
    """
    ...
    os.system("ffmpeg -framerate 12 -start_number 0 -i "+nbuddies_path+"/movie_dump"+ brute_or_tree+"/trajectories_%01d.png -q:v 0 "+nbuddies_path+f"/trajectories_{tot_nstep_eta}.mkv") # recreate movie

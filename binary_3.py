...
# this is the final wrapper function built by timesteps team, and will call inside it 
# the update_x, compute_force and update_v functions. We are only interested in the output BH_data_final
def wrapper_func_for_evolution(BH_data_ic, delta_t, tot_time, f):
    """
    refer discord for documentation
    returns - 
    BH_data_final: list of BH_objects with each having position and velocity attributes
                   that are M x 3 np arrays (M is the total no. of timesteps = tot_time/delta_t)
    """
    # code supplied by the timesteps team
...
#######################################################
    ...
    def transform_coords(self):
        ...
        r1_vec = BH_data_final[0].position  
        r2_vec = BH_data_final[1].position
        ...
        v1_vec = BH_data_final[0].velocity
        v2_vec = BH_data_final[1].velocity   # M x 3 arrays 

        relative_v_vec = v1_vec - v2_vec  # M x 3 array 

        m_1 = BH_data_final[0].mass 
        m_2 = BH_data_final[1].mass # scalar
...
############################################################
    ...
    energy = 1/2*self.mu*np.sum(self.relative_v_vec**2, axis = 1) + ang_mom**2/(2*self.mu*np.sum(self.relative_r_vec**2, axis = 1)) - G*m_1*m_2/np.linalg.norm(self.relative_r_vec, axis = 1)

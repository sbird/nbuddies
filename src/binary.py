...
def generate_binary_ICs(N_BH, custom_vals = None, analy_sets = False):
    ...
    if custom_vals != None:
        ...
        # clean directory
        if os.path.exists(nbuddies_path+'/data/binary'):
            shutil.rmtree(nbuddies_path+'/data/binary') 
        os.makedirs(nbuddies_path+'/data/binary')
        ## Save the file
        with open(nbuddies_path+'/data/binary/BH_data_binary.pkl', 'wb') as handle:

...
def test_dynamics(): # Test gravitational forces between black holes earth_like and sun_like
    ...
    bh1_earth = BlackHole(mass=3.0e-6, position=(4.848136811133344e-09, 0, 0), velocity=(0, 29.78, 0))  # Earth-like BH, position is 1 AU in kpc
    ...
    t = comp_adaptive_dt(a, j, s, eta=0.1, tot_time = 31557.6)  # time value is 1 mili-yr in sec (putting manually instead of pint)

    # Assertion check
    #assert abs(actual - expected)/expected <= tolerance

    accel_error = np.abs(np.linalg.norm(a)/expected_acceleration - 1)
    jerk_error = np.abs(np.linalg.norm(j)/expected_jerk - 1)
    snap_error = np.abs(np.linalg.norm(s)/expected_snap - 1)
    time_step_error = np.abs(t/expected_dt - 1)

...
# A simple class to store black hole properties
class BlackHole:
    def __init__(self, mass, position, velocity): # __init__ runs automatically when you create an object of the class
        self.mass = mass  # in solar masses
        self.position = np.array(position)  # in kpc (3D vector) #numpy array with 3 components [x,y,z]
        self.velocity = np.array(velocity)  # in km/s (3D vector) #numpy array with 3 components [vx,vy,vz]
...
def (n_blackholes, box_size, mass_range = (3, 10**11), vel_scale_kms = 100): # a function to create n_blackholes with masses, positions, and velocities, box_size in kpc (side length of the cubic box)
